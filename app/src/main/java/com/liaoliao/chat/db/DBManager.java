...
import com.liaoliao.chat.activity.login.LoginActivity;
import com.liaoliao.chat.api.ApiRetrofit;
import com.liaoliao.chat.base.AppConst;
import com.liaoliao.chat.db.model.Friend;
...
import com.liaoliao.chat.utils.NetUtils;
                    ...
                    .subscribe(getGroupInfoResponse -> {
                        if (getGroupInfoResponse != null && getGroupInfoResponse.getCode() == 200) {
                            ...
                            if (groupInfo != null) {
                                ...
                                String role = groupInfo.getCreatorId().equalsIgnoreCase(new Setting(App.getContext()).loadString("userId")) ? "0" : "1";
...
//        deleteGroupMembers();
    ...
    public boolean isMe(String userid) {
        if (new Setting(App.getContext()).loadString("userId").equalsIgnoreCase(userid)) {
            return true;
        }
        return false;
    }
...
//        return DataSupport.findAll(Friend.class);
        return DataSupport.where("userid != ?", new Setting(App.getContext()).loadString("userId")).find(Friend.class);
    }
